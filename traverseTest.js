...
(function (angular) {
     ...
     */
    function myCartControllerConstructor($log, loggingService, $rootScope, cartHttpDataService, creditClassService,
			 commonUtils, $timeout, cartControllerHelperService, CartCommonModal, cartStorage,
			 deviceProtectionAndServicesHttpData, keyValues, globalConfig, globalValue, jsonPath, cartPreviewService,
			 cartPlanSelectorHttpDataService, tmobNotificationService, checkOutValidationUtil, cartAccessoryService,
			 cartPromoCodeService, $q) {
        ...
        function addDeviceToLine(modalData, selectedLine, oldDevice, newDevice) {
            ...
            if (modalData.SKU === 'SIM_KIT_BYOD' && !angular.equals(modalData.SKU, oldDevice)) {
                cartHttpDataService.removeDeviceFromLine(selectedLine.lineId, selectedLine.lineItemDetails.deviceDetails.id)
                .then(function (removeDeviceStatus) {
                    if (!angular.equals(removeDeviceStatus, 200)) {
                        loggingService.error('We have an error adding device to cart.');
                    } else {
                        cartHttpDataService.addToLine(requestBody, selectedLine.lineId)
                        .then(function (cartResponseStatus) {
        ...
        function removeServicesFromCart(selectedLine) {
            deviceProtectionAndServicesHttpData.getServices(selectedLine.lineId)
            .then(function (serviceResponse) {
                cartStorage.setServiceId(serviceResponse.data.services.id, selectedLine.lineId);
                deviceProtectionAndServicesHttpData.addServicesToLine(selectedLine.lineId)
                .then(function (cartResponseStatus) {
                    if (!angular.equals(cartResponseStatus, 200)) {
                        ...
                        selectedLine.mvcAttrs.isDeviceChanged = true;
                        selectedLine.mvcAttrs.isServiceChanged = false;
                        if (selectedLine.lineItemDetails.serviceDetails &&
			         selectedLine.lineItemDetails.serviceDetails.length > 0) {
        ...
        function checkDeviceLimit() {
            if (vm.structCartData) {
                if (vm.structCartData.lineItems.length === 0) {
                    ...
                    vm.lineMaxHandsetLimitReached = lineItemsHandsetArray &&
			 lineItemsHandsetArray.length >= maxHandsetLength ? true : false;
                    vm.lineMaxTabletLimitReached = lineItemsTabletArray &&
			 lineItemsTabletArray.length >= maxTabletLength ? true : false;
                    ...
                    vm.deviceLimitReached = vm.lineMaxHandsetLimitReached &&
			 vm.lineMaxTabletLimitReached ||
			 vm.structCartData.lineItems.length >= length ? true : false;
        ...
        /*
         * if (creditClassService.getEPCreditClass() === 'NONE' &&
			 vm.structCartData.lineItems.length >= length) { vm.deviceLimitReached = true; } else {
         * vm.deviceLimitReached = false; }
         */
        /*
         * <Function description> 
        ...
        function addLineByLineType(lineType) {
            ...
            cartHttpDataService.addNewLine(requestBody)
            .then(function (cartResponseStatus) {
                if (!angular.equals(cartResponseStatus, 200)) {
                    loggingService.error('We have an error adding new line to cart.');
                } else {
        ...
        function checkDuplicateDisable(selectedLine) {
            ...
            if (vm.deviceLimitReached || selectedLine.lineType === 'TABLET' &&
			 vm.lineMaxTabletLimitReached || selectedLine.lineType === 'HANDSET' &&
			 vm.lineMaxHandsetLimitReached) {
        ...
        function duplicateLine(selectedLine) {
            ...
            cartHttpDataService.duplicateLine(selectedLine.lineId)
            .then(function (cartResponseStatus) {
                if (!angular.equals(cartResponseStatus, 200)) {
                    loggingService.error('We have an error adding new line to cart.');
                } else {
        ...
        function removeLine(selectedLine) {
            ...
            cartHttpDataService.removeLine(lineId)
            .then(function (cartResponseStatus) {
                if (!angular.equals(cartResponseStatus, 200)) {
        ...
        function toggleSelectedPlan(selectedLine) {
            cartHttpDataService.toggleSelectedPlan(selectedLine)
        ...
        function MiniPDPModalCallBack(modalData) {
            ...
            if (_selectedLine.simKitDetails && _selectedLine.simKitDetails.isProductAdded &&
			 angular.equals('SIM_KIT_BYOD', _selectedLine.simKitDetails.simKitId)) {
                addDeviceToLine(modalData, modalData.selectedLine, 'SIM_KIT_BYOD', cartStorage.getDeviceFamily(devicePlans.device));
            } else if (_selectedLine.lineItemDetails &&
			 _selectedLine.lineItemDetails.deviceDetails &&
			 _selectedLine.lineItemDetails.deviceDetails.isProductAdded) {
                if (cartSettings && cartSettings.removeServiceInEditDevice) {
                    ...
                    if (angular.equals(cartSettings.removeServiceInEditDevice, cartSettings.constants.skuId)) {
                        addDeviceToLine(modalData, modalData.selectedLine, _selectedLine.lineItemDetails.deviceDetails.sku,
			 devicePlans.device);
        ...
        function deviceSelectorModalCallBack(modalData) {
            ...
            if (modalData.openminipdppage) {
                var _commonResolves = cartControllerHelperService.commonResolves('miniPDPModalView', _selectedLine,
			 1, keyValues, null, modalData.deviceData);
                var resolveAttributes = { resolve: angular.extend(_commonResolves.openView, _commonResolves.selectedCreditClass,
			 _commonResolves.selectedProductSKU, _commonResolves.keyValues, _commonResolves.selectedLine,
			 _commonResolves.selectedDeviceOfferId) };
			 ...
			 _selectedLine.lineItemDetails.deviceDetails.isProductAdded) {
                    if (cartSettings && cartSettings.removeServiceInEditDevice) {
                        // Sku ID
                        if (angular.equals(cartSettings.removeServiceInEditDevice, cartSettings.constants.skuId)) {
                            addDeviceToLine(modalData, selectedLine, _selectedLine.lineItemDetails.deviceDetails.sku,
			 devicePlans.device);
                        } else if (angular.equals(cartSettings.removeServiceInEditDevice, cartSettings.constants.familyId)) {
        ...
        function openDeviceSelectorModal(openView, selectedLine, i) {
            var _commonResolves = cartControllerHelperService.commonResolves(openView, selectedLine, i,
			 keyValues, null);
            var resolveAttributes = { resolve: angular.extend(_commonResolves.openView, _commonResolves.selectedCreditClass,
			 _commonResolves.selectedProductSKU, _commonResolves.keyValues, _commonResolves.selectedLine) };
         ...
         * @function addPlan
         * @name addPlan
         * @param {String} selectedLine selected line is a single of the cart which consist of line item,
			plan and protection plans.
         */
        function addPlan(selectedLine) {
            cartPlanSelectorHttpDataService.submitSelectedPlan()
            .then(function (planOptionCartData) {
                vm.structCartData = cartStorage.getGlobalSmartCartInfo();
        ...
        function showCartPreview(selectedLine, planOption) {
            ...
            cartPlanSelectorHttpDataService.selectPlan(data)
            .then(function (planOptionCartData) {
        ...
        /*
                 ...
                 return cartPlanSelectorHttpDataService.selectPlan(data)
            .then(function(planOptionCartData) {
                 cartStorage.setGlobalSmartCartInfo(baseCartObj);
                 vm.structCartData = cartStorage.getGlobalSmartCartInfo();
                 });
        ...
        function openPriceBreakdownModal() {
            ...
            //    return;
            // }
            var _commonResolves = cartControllerHelperService.commonResolves(null, vm.structCartData,
			 null, keyValues);
        ...
        function openDeviceProtectionAndServicesSelectorModal(selectedLine, index) {
            var commonResolves = cartControllerHelperService.commonResolves(null, selectedLine, index,
			 keyValues);
            var serviceCount = selectedLine.lineItemDetails.serviceDetails.length;
            var resolveAttributes = { resolve: angular.extend(commonResolves.keyValues, commonResolves.selectedProtectionPlans,
			 commonResolves.selectedLine) };
            ...
            CartCommonModal.openCartModal('deviceProtectionAndServicesSelectorModal', resolveAttributes,
			 modalCallBack);
        }
        /*
         * <Function description> 
        ...
        function openOwnPhoneModal(selectedLine) {
            var _commonResolves = cartControllerHelperService.commonResolves(null, selectedLine, null,
			 keyValues);
            var resolveAttributes = { resolve: angular.extend(_commonResolves.keyValues, _commonResolves.selectedLine) };
            // TODO: extract function up to controller private function and reference in here.
            var modalCallBack = function (modalObject) {
                ...
                if (angular.equals(modalObject.phoneDetails.SKU, 'SIM_KIT_BYOD')) {
                    if (_selectedLine.lineItemDetails.deviceDetails &&
			 _selectedLine.lineItemDetails.deviceDetails.isProductAdded) {
                        addDeviceToLine(modalObject.phoneDetails, selectedLine, _selectedLine.lineItemDetails.deviceDetails.sku,
			 modalObject.phoneDetails.SKU);
                        ...
                        addDeviceToLine(modalObject.phoneDetails, selectedLine, modalObject.phoneDetails.SKU,
			 modalObject.phoneDetails.SKU);
        ...
        function openCreditClassSelectorModal(current, callback) {
            ...
            var resolveAttributes = { resolve: angular.extend(commonResolves.selectedCreditClass, commonResolves.source,
			 commonResolves.isPlanSelectorModal, commonResolves.keyValues, commonResolves.selectedLine) };
            ...
            var modalCallBack = function (modalObject) {
                ...
                if (!vm.structCartData.selectedCreditClass ||
			 !angular.equals(vm.structCartData.selectedCreditClass['class'], modalObject.selectedCreditClass['class'])) {
                    ...
                    cartHttpDataService.getCart().then(function (cartResponseStatus) {
                        /*
                         * If we initially select No Credit Check, add a line and then change the credit class to Awesome/Average,
			 it shows that device limit is reached.
        ...
        function isAccessoryAddedToCart() {
            vm.structCartData = cartStorage.getGlobalSmartCartInfo();
            if (angular.isDefined(vm.structCartData.accessories) &&
			 vm.structCartData.accessories.accessoryDetails.length > 0) {
                return true;
            }
            return false;
        }
        ...
        function initialize() {
            ...
            if (authorValueFlagEnable) {
                if (vm.authorValue && 'cart' in vm.authorValue) {
                    ...
                    if ('creditSelector' in vm.keyValues &&
			 'class' in vm.keyValues.creditSelector) {
                        vm.keyValues['class'] = [];
            ...
            if (cartSettings && !cartSettings.isEmptyCartEnabled &&
			 vm.structCartData && vm.structCartData.flags &&
			 !vm.structCartData.flags.isCartReloaded) {
                addLine();
            }
